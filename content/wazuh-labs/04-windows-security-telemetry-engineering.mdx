...
**PowerShell Operational** — No alert appeared in the Wazuh dashboard for this specific event, consistent with the alert-visibility gap identified in Part 3. However, the underlying Event ID 4104 was confirmed locally in Event Viewer, and its message body showed the **fully decoded** script content — `Get-LocalUser | Select-Object Name` — rather than the base64 string that was actually typed on the command line.
...
PowerShell Operational sat in the middle: the underlying event existed and was confirmed locally in Event Viewer, but it did not surface as an alert in the Wazuh dashboard, since it didn't match any rule severe enough to be indexed. Where it earned its place, however, was in content — it was the only source that revealed the fully decoded script text rather than the encoded string actually typed on the command line.
